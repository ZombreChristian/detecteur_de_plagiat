...
app = FastAPI(title="DOCSEC - Détection documentaire", version="1.2.0")
...
def run_detection(file_path, kind):
    ...
    return {
        ...
        "sources": [{k: v for k, v in item.items() if k != "_path"} for item in ranked[:10]],
...
@app.get("/api/health")
def health():
    return {"status": "ok", "corpora": {kind: len(files_in_corpus(kind)) for kind in CORPORA}}
